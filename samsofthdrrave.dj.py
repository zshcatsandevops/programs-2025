...
try:
    ...
    if sys.version_info >= (3, 13):
        import audioop as _audioop  # noqa: F401 - ensure import works at runtime
...
def download_audio(urls: list[str], output_dir: str) -> list[str]:
    ...
    ydl_opts = {
        ...
        "overwrites": True,
        "ignoreerrors": True,  # safer cross-version than "only_download"
        "quiet": True,
        "no_warnings": True,
        "noprogress": True,
    }
...
def generate_mashups(audio_files: list[str], num_mashups: int, output_dir: str):
    ...
    for i in range(1, num_mashups + 1):
        ...
        for _ in range(layers):
            ...
            # Position within current mashup length (0 if overlay is longer)
            max_pos = max(0, len(mashup) - len(seg))
            pos = random.randint(0, max_pos) if max_pos > 0 else 0
...
def get_urls_from_text(text_widget: tk.Text) -> list[str]:
    ...
    return [u.strip() for chunk in raw.split(",") for u in chunk.split() if u.strip()]
