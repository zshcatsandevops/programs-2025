...
# ───────────── Config ─────────────
GAME_TITLE = "ULTRA MARIO 2D BROS v2.0"
...
class Player(Entity):
    def __init__(self,x,y):
        ...
        self.coyote=self.buf=0; self.inv=0; self.game=None; self.fire=False
...
class Game:
    ...
    def load(self,w,s):
        ...
        self.state="play"; self.cam_x=0; self.time_left=START_TIME
    ...
    def draw_title(self,s):
        ...
        s.blit(t,(BASE_W//2-t.get_width()//2,60))
        if int(self.title_blink*2)%2==0:
            m=fhud.render("Press ENTER or Z to Start",True,c['white'])
            s.blit(m,(BASE_W//2-m.get_width()//2,130))
        lines=["← / → MOVE","Z/SPACE JUMP","SHIFT RUN","X FIREBALL","P PAUSE","R RESTART","F11 FULLSCREEN","F5 SCALE","ESC QUIT"]
        for i,l in enumerate(lines):
            t=fhud.render(l,True,c['white']); s.blit(t,(BASE_W//2-t.get_width()//2,150+i*10))
        foot=fhud.render("© Samsoft 2025   © 1985 Nintendo (Tribute)",True,c['white'])
        s.blit(foot,(BASE_W//2-foot.get_width()//2,BASE_H-20))
