...
flagpole, enemies, and HUD. Enhanced parser for better level feel/vibe matching NES footage.
...
# ───────── LEVEL DATA ─────────
# Terrain object data from SMB1 disassembly (Gist and 6502disassembly.com)
# Expanded with more levels from known labels; placeholders for others.
LEVEL_OBJECT_DATA = {
...
LEVEL_WIDTH_TILES = 96  # 48 metatiles * 2 (scaled)

def parse_object_data(data, floor_y):
    """Enhanced parser for SMB1 object data, matching NES feel with more IDs from disassembly."""
    tiles = []
    i = 0
    col = 0  # Metatile column, scaled *2 for TILE
    while i < len(data) - 1:
        obj_id = data[i]
        i += 1
        if i >= len(data):
        ...
        if obj_id == 0xfd:
            break  # End of data
        elif obj_id == 0x02:  # Row of bricks (large)
            length = param & 0x0f
            y = floor_y - TILE
            for l in range(length):
                x = col * 2 * TILE + l * TILE
                tiles.append((x, y, BROWN))  # Bricks brown
            col += length
        elif obj_id == 0x03:  # Row of solid blocks
            length = param & 0x0f
            y = floor_y - TILE
            for l in range(length):
                x = col * 2 * TILE + l * TILE
                tiles.append((x, y, BROWN))  # Solid brown
            col += length
        elif obj_id == 0x04:  # Row of coins
            length = param & 0x0f
            y = floor_y - TILE
            for l in range(length):
                x = col * 2 * TILE + l * TILE
                tiles.append((x, y, YELLOW))  # Coins yellow
            col += length
        elif obj_id == 0x05:  # Column of bricks (small)
            height = param & 0x0f + 1
            x_start = col * 2 * TILE
            for h in range(height):
                y = floor_y - (h + 1) * TILE
                tiles.append((x_start, y, BROWN))
            col += 1
        elif obj_id == 0x06:  # Column of solid blocks or brick with item
            height = param & 0x0f + 1
            x_start = col * 2 * TILE
            for h in range(height):
                y = floor_y - (h + 1) * TILE
                color = BLUE if h == 0 else BROWN  # Top as ? block
                tiles.append((x_start, y, color))
            col += 1
        elif obj_id == 0x07:  # Brick with coins (small row)
            length = param & 0x0f
            obj_type = (param >> 4)
            y = floor_y - TILE
            ...
            col += 2
        elif obj_id == 0xa7:  # Small staircase
            # Approx 3-4 steps
            for s in range(4):
                x = col * 2 * TILE + s * TILE
                y = floor_y - s * TILE
                tiles.append((x, y, BROWN))
            col += 4
        elif obj_id == 0xb3:  # Large staircase
            # Approx 7 steps
            for s in range(7):
                x = col * 2 * TILE + s * TILE
                y = floor_y - s * TILE
                tiles.append((x, y, BROWN))
            col += 7
        elif obj_id == 0xb7:  # Large staircase variant
            for s in range(7):
                x = col * 2 * TILE + s * TILE
                y = floor_y - s * TILE
                tiles.append((x, y, BROWN))
...
def generate_level(num):
    ...
    tiles_list = []
    floor_y = H - TILE * 2
    # Ground as brown dirt for NES vibe
    for x in range(LEVEL_WIDTH_TILES):
        tiles_list.append((x * TILE, floor_y, BROWN))
    # Parse objects
...
def game_loop(level_num):
    ...
    while True:
        ...
        screen.fill((92,148,252))  # Sky blue for NES vibe
        # Simple background hill for ground levels (vibe match)
        hill_points = [(50 - camera_x % 200, H-100), (150 - camera_x % 200, H-200), (250 - camera_x % 200, H-100)]
        if level_num % 4 != 0:  # Ground areas
            pygame.draw.polygon(screen, GREEN, hill_points)
        for t in tiles:
