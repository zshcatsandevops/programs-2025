...
DARK_BLUE_BG = (0, 0, 50)  # Menu background approximation
...
def world_to_tile(world_x, world_y):
    """Convert tile coordinates to tile coordinates"""
...
class Game:
    ...
    def __init__(self):
        ...
        self.copyright_font = pygame.font.Font(None, 20)
        self.running = True
        self.state = 'menu'  # 'menu' or 'playing'
        self.selected = 0  # 0: 1 player, 1: 2 player
        self.menu_frame = 0
        
        # Game state (initialized on menu exit)
        self.world = None
        self.mario = None
        self.camera_x = 0
    ...
    def draw_menu(self):
        """Draw the main menu approximating the NES title screen"""
        self.screen.fill(DARK_BLUE_BG)
        
        # Title banner background
        banner_rect = pygame.Rect(100, 50, 568, 80)
        pygame.draw.rect(self.screen, BROWN, banner_rect)
        
        # Title text
        title = self.title_font.render("ULTRA MARIO 2D BROS.", True, PINK)
        title_rect = title.get_rect(center=banner_rect.center)
        self.screen.blit(title, title_rect)
        
        # Copyright
        copyright = self.copyright_font.render("\u00A9 1985 NINTENDO \u00A9 2025 SAMSOFT", True, WHITE)
        copyright_rect = copyright.get_rect(center=(DISPLAY_W // 2, 140))
        self.screen.blit(copyright, copyright_rect)
        
        # Player options with cursor
        y_pos = 200
        for i in range(2):
            cursor = "\u25CF " if i == self.selected else "  "
            text = self.option_font.render(f"{cursor}{i+1} PLAYER GAME", True, WHITE)
            text_rect = text.get_rect(center=(DISPLAY_W // 2, y_pos))
            self.screen.blit(text, text_rect)
            y_pos += 40
        
        # FILES = OFF
        files_text = self.option_font.render("FILES = OFF", True, WHITE)
        files_rect = files_text.get_rect(center=(DISPLAY_W // 2, y_pos + 20))
        self.screen.blit(files_text, files_rect)
        
        # TOP score
        top_text = self.option_font.render("TOP- 000000", True, WHITE)
        top_rect = top_text.get_rect(center=(DISPLAY_W // 2, 350))
        self.screen.blit(top_text, top_rect)
        
        # Bottom score line
        bottom_text = self.option_font.render("000000    x00    1-1", True, WHITE)
        bottom_rect = bottom_text.get_rect(center=(DISPLAY_W // 2, 380))
        self.screen.blit(bottom_text, bottom_rect)
        
        # Hill
        hill_points = [(400, 500), (500, 420), (600, 500)]
        pygame.draw.polygon(self.screen, GREEN, hill_points)
        
        # Mario on hill (small, facing right, animated)
        mario_unscaled_x = 450 / SCALE  # Adjust for scale
        mario_y = 420 - SMALL_MARIO_HEIGHT
        SpriteRenderer.draw_mario(self.screen, mario_unscaled_x, mario_y, False, False, True, self.menu_frame)
        
        # Clouds (simple polygons)
        cloud1_points = [(100, 150), (130, 130), (160, 150), (130, 170)]
        pygame.draw.polygon(self.screen, WHITE, cloud1_points)
        cloud2_points = [(600, 200), (630, 180), (660, 200), (630, 220)]
        pygame.draw.polygon(self.screen, WHITE, cloud2_points)
        
        # Bushes
        bush1_points = [(650, 480), (670, 460), (690, 480), (670, 500)]
        pygame.draw.polygon(self.screen, DARK_GREEN, bush1_points)
    ...
    def run(self):
        ...
        print("Menu Controls:")
        print("  Up/Down - Select player mode")
