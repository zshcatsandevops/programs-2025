...
Modified to add physics-based bump animation to all bumpable blocks (e.g., '?', 'B', '#', 'G').
Blocks now simulate physical motion when hit from below, with velocity and gravity for realistic bouncing.
Effects (coin pop, power-up spawn, brick break) are timed to the physics simulation (e.g., break at peak, spawn at bottom).
...
# NEW: Coin pop (little animation), Fire flower power-up, lightweight brick debris
...
# NEW: Physics-based bump for blocks
class Bump(Entity):
    def __init__(self, gx, gy, ch):
        super().__init__(gx * TILE, gy * TILE, TILE, TILE)
        self.gx = gx
        self.gy = gy
        self.ch = ch
        self.orig_y = self.rect.y
        self.vy = -280.0  # initial upward velocity for bump
        self.grav = 4200.0  # accelerated gravity for quick return
    ...
    def update(self, g, dt):
        ...
        self.rect.y += int(self.vy * dt)
        if prev_vy < 0 and self.vy >= 0:
            # At peak
            if self.ch == 'B':
                for (dx, dyv) in [(-60, -280), (60, -280), (-40, -200), (40, -200)]:
                    g.entities.append(Debris(self.rect.x + 5, self.rect.y + 5, dx, dyv))
                g.player.score += 50
                self.remove = True
                return
        if self.rect.y >= self.orig_y:
            ...
            if self.ch == '?':
                g.level._set(self.gx, self.gy, 'G')
            elif self.ch == 'B':
                pass  # already broken
            else:
                g.level._set(self.gx, self.gy, self.ch)
            if self.has_power:
                g.entities.append(FireFlower(self.gx * TILE + 2, (self.gy - 1) * TILE - 8))
            self.remove = True

    def draw(self, g, s, cx):
        if self.ch:
            s.blit(g.tiles.tile(self.ch), self.rect.move(-cx, 0))
...
class Game:
    def __init__(self):
        ...
        self.level=None; self.player=None; self.entities: List[Entity] = []
    ...
    # MODIFIED: block bump with physics
    def bump_block(self, gx, gy):
        assert self.level and self.player
        ch = self.level.tile(gx, gy)
        if ch not in "B?#G#": return  # bumpable blocks
        bump = Bump(gx, gy, ch)
        if ch == '?':
            self.entities.append(CoinPop(gx * TILE + 4, gy * TILE - TILE // 2))
            self.player.coins += 1
            self.player.score += 200
            if self.player.coins >= 100:
                self.player.coins -= 100
                self.player.lives += 1
            bump.has_power = random.random() < 0.40
        self.level._set(gx, gy, ' ')
        self.entities.append(bump)
