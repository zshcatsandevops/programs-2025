#!/usr/bin/env python3
"""
SUPER MARIO BROS 1 ENGINE - MODERN HUD EDITION
Faithful SMB1 mechanics with modern Team Level Up style HUD
...
pygame.display.set_caption("Super Mario Bros 1 Engine - Modern HUD Edition")
clock = pygame.time.Clock()
FPS = 60

# -----------------------------------------------------------------------------
# Colors (NES SMB1 Palette + Modern HUD Colors)
# -----------------------------------------------------------------------------
...
# Modern HUD colors
HUD_BG = (40, 40, 60, 200)  # Semi-transparent dark blue
HUD_ACCENT = (0, 180, 255)   # Bright blue accent
HUD_TEXT = (240, 240, 240)   # Off-white text
...
# -----------------------------------------------------------------------------
# Modern HUD (Team Level Up Style)
# -----------------------------------------------------------------------------
class ModernHUD:
    def __init__(self):
    ...
    def update(self):
        # Update animations
        self.score_pulse = max(0, self.score_pulse - 1)
        self.coin_spin = (self.coin_spin + 1) % 360
        self.life_pulse = max(0, self.life_pulse - 1)
        
    def add_score(self, points):
        self.score += points
        self.score_pulse = 20  # Pulse effect
    ...
    def add_coin(self):
        ...
        if self.coins >= 100:
            self.coins = 0
            self.lives += 1
            self.life_pulse = 30  # Pulse effect for extra life
            
    def draw(self, surface):
        # Draw HUD background (semi-transparent panel)
        hud_rect = pygame.Rect(10, 10, NATIVE_WIDTH - 20, 50)
        s = pygame.Surface((hud_rect.width, hud_rect.height), pygame.SRCALPHA)
        pygame.draw.rect(s, HUD_BG, (0, 0, hud_rect.width, hud_rect.height), border_radius=8)
        pygame.draw.rect(s, HUD_ACCENT, (0, 0, hud_rect.width, hud_rect.height), 2, border_radius=8)
        surface.blit(s, hud_rect)
        
        # Score display with pulse effect
        score_color = HUD_TEXT
        if self.score_pulse > 0:
            pulse_intensity = 50 + int(205 * (self.score_pulse / 20))
            score_color = (pulse_intensity, pulse_intensity, 255)
            
        score_label = self.font_small.render("SCORE", True, HUD_ACCENT)
        score_text = self.font_large.render(f"{self.score:06d}", True, score_color)
        surface.blit(score_label, (20, 15))
        surface.blit(score_text, (20, 30))
        
        # Coins display with spinning coin
        coin_radius = 8
        coin_x = 120
        coin_y = 35
        
        # Animated coin
        coin_size_mod = math.sin(math.radians(self.coin_spin * 4)) * 2
        pygame.draw.ellipse(surface, COIN_GOLD, 
                           (coin_x - coin_radius - coin_size_mod/2, 
                            coin_y - coin_radius - coin_size_mod/2,
                            coin_radius*2 + coin_size_mod, 
                            coin_radius*2 + coin_size_mod))
        
        coin_label = self.font_small.render("COINS", True, HUD_ACCENT)
        coin_text = self.font_large.render(f"x{self.coins:02d}", True, HUD_TEXT)
        surface.blit(coin_label, (110, 15))
        surface.blit(coin_text, (140, 30))
        
        # World display
        world_label = self.font_small.render("WORLD", True, HUD_ACCENT)
        world_text = self.font_large.render(self.world, True, HUD_TEXT)
        surface.blit(world_label, (200, 15))
        surface.blit(world_text, (200, 30))
        
        # Lives display with pulse effect on extra life
        lives_color = HUD_TEXT
        if self.life_pulse > 0:
            pulse_intensity = 50 + int(205 * (self.life_pulse / 30))
            lives_color = (pulse_intensity, 255, pulse_intensity)
            
        lives_label = self.font_small.render("LIVES", True, HUD_ACCENT)
        lives_text = self.font_large.render(f"{self.lives}", True, lives_color)
        surface.blit(lives_label, (250, 15))
        surface.blit(lives_text, (250, 30))
        
        # Time display with warning color when low
        time_color = HUD_TEXT
        if self.time < 100:
            time_color = (255, 100, 100)  # Red when time is low
            
        time_label = self.font_small.render("TIME", True, HUD_ACCENT)
        time_text = self.font_large.render(f"{self.time}", True, time_color)
        surface.blit(time_label, (300, 15))
        surface.blit(time_text, (300, 30))
        
        # Power-up status (if implemented)
        power_label = self.font_small.render("POWER", True, HUD_ACCENT)
        power_text = self.font_tiny.render("NORMAL", True, HUD_TEXT)
        surface.blit(power_label, (350, 15))
        surface.blit(power_text, (350, 32))
    ...
    while running:
        ...
        if state == "menu":
            ...
            for i, text in enumerate(controls):
                ...
                game_surface.blit(control_text, (NATIVE_WIDTH//2 - control_text.get_width()//2, 130 + i * 20))
