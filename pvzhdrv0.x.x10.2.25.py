...
def samsoft_intro():
    ...
    angle = 0
    # SOUND REMOVED: play_seq([(220, 0.18), (440, 0.18), (880, 0.22)])
...
def ea_logo():
    ...
    alpha, angle = 0, 0
    # SOUND REMOVED: play_seq([(220, 0.26), (330, 0.26), (440, 0.32)])
    for frame in range(180):
        safe_quit_check()
        screen.fill(BLACK)
...
def popcap_logo():
    ...
    alpha, angle = 0, 0
    # SOUND REMOVED: play_seq([(440, 0.12), (660, 0.12), (880, 0.12), (1320, 0.12), (990, 0.22)])
...
class Game:
    ...
    def reset(self):
        ...
        self.tutorial_steps = [
            ...
            "We're going BOWLING!",
            "HERE, TAKE THIS WALL-NUT!!",
            "Why'd I put a wall-nut in your hand?",
            "Because I'm CRAAAZY!!!!!",
            "NOW GO! BOWL ME A WINNER!"
        ]  # Full dialog sequence from original PVZ
        self.tutorial_active = True  # Start with tutorial paused
        ...
        while running:
            ...
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    ...
                    # Tutorial advance
                    if self.tutorial_active and e.key == pygame.K_SPACE:
            ...
            # Update
            if self.state == "play" and not self.tutorial_active:
                self.update(dt)
            else:
                self.state_time += dt
    ...
    # New: Draw tutorial with Crazy Dave
    def draw_tutorial(self, surf):
        ...
        # Draw Crazy Dave (procedural: left side)
        dave_x, dave_y = 120, SCREEN_HEIGHT // 2 + 50
        # Body rect
        body_rect = pygame.Rect(dave_x - 60, dave_y + 20, 120, 140)
        pygame.draw.rect(surf, (200, 200, 200), body_rect, border_radius=10)  # Shirt
        # Head circle
        head_center = (dave_x, dave_y - 40)
        pygame.draw.circle(surf, (255, 220, 180), head_center, 50)  # Face
        # Eyes
        pygame.draw.circle(surf, WHITE, (dave_x - 20, dave_y - 50), 10)
        pygame.draw.circle(surf, WHITE, (dave_x + 20, dave_y - 50), 10)
        pygame.draw.circle(surf, BLACK, (dave_x - 20, dave_y - 50), 4)
        pygame.draw.circle(surf, BLACK, (dave_x + 20, dave_y - 50), 4)
        # Beard (simple lines)
        for i in range(10):
            bx1 = dave_x - 30 + i * 6
            by1 = dave_y - 20
            bx2 = bx1 + random.randint(-10, 10)
            by2 = by1 + 40
            pygame.draw.line(surf, (150, 100, 50), (bx1, by1), (bx2, by2), 3)
        # Pot hat
        pot_rect = pygame.Rect(dave_x - 40, dave_y - 100, 80, 40)
        pygame.draw.rect(surf, (150, 150, 150), pot_rect, border_radius=5)
        handle = pygame.Rect(dave_x - 70, dave_y - 90, 30, 10)
        pygame.draw.rect(surf, (150, 150, 150), handle)
        
        # Dialog bubble
        bubble_rect = pygame.Rect(200, 150, 400, 200)
        pygame.draw.rect(surf, WHITE, bubble_rect, border_radius=20)
        pygame.draw.polygon(surf, WHITE, [(200, 220), (160, 250), (200, 280)])  # Tail to Dave
        
        # Current dialog text
        if self.tutorial_index < len(self.tutorial_steps):
            text = FONT_MED.render(self.tutorial_steps[self.tutorial_index], True, BLACK)
            surf.blit(text, (bubble_rect.x + 20, bubble_rect.y + 20))
        
        # Prompt to continue
        prompt = FONT_SMALL.render("Hit SPACE to continue...", True, WHITE)
        surf.blit(prompt, (SCREEN_WIDTH // 2 - prompt.get_width() // 2, SCREEN_HEIGHT - 50))
