...
Adventure + Mini-Games (stub) + Almanac + Options + Controls + Quit
One playable lawn level with basic PVZ-like mechanics (original placeholder art).
Now with ESC-based Pause Menu and minor optimizations.
© Samsoft 2025
"""
...
# ───────── UTIL ─────────
SKY_SURF = None
def init_sky_surface():
    """Pre-render the vertical gradient sky once (small perf win)."""
    global SKY_SURF
    SKY_SURF = pygame.Surface((W, H))
    for y in range(H):
        r=[int(SKY_TOP[i]+(SKY_BOTTOM[i]-SKY_TOP[i])*y/H) for i in range(3)]
        pygame.draw.line(SKY_SURF, r, (0,y), (W,y))

def draw_day_sky():
    """Blit the pre-rendered sky gradient."""
...
def draw_grass_band(t):
    ...
    # tuft wobble
    sin, rng = math.sin, range
    for i in rng(0,W,15):
        h=16+10*sin(t*4+i*0.1)
...
def draw_glow_text(text,color,glow=(255,255,200),size=64,pulse=1,pos=None):
    """Renders text with a soft pulsing glow effect (menu/title use)."""
...
class MainMenu:
    ...
    def __init__(self, app):
        self.app=app
        bx=W//2-90
        by=110  # adjusted to fit 6 buttons on 400px high window
        self.buttons=[
            ...
            Button((bx,by+150,180,40),"Controls",lambda: app.go(Controls(app))),
            Button((bx,by+200,180,40),"Options",lambda: app.go(Options(app))),
            Button((bx,by+250,180,40),"Quit",quit_now)
        ]
        self.phase=0
    def update(self,dt):
        self.phase+=dt
    def draw(self,t):
        ...
        # Menu panel (sized for 6 buttons)
        pygame.draw.rect(screen,WOOD_DARK,(W//2-110,100,220,220),border_radius=12)
        pygame.draw.rect(screen,WOOD_LIGHT,(W//2-110,100,220,220),3,border_radius=12)
...
class Options:
    ...
    def __init__(self, app, on_back=None):
        ...
        self.back=Button((20,H-60,160,38),"Back",self.back_action)
        self.sync_labels()
...
class Controls:
    """Simple static controls/help screen (also used from Pause)."""
    LINES = [
        "Controls",
        "",
        "Mouse Left: select seed • collect sun • place plant",
        "Mouse Right: cancel selection",
        "Keys 1–5: quick-select seed packet",
        "ESC: Pause/Resume gameplay (in game) • Back (in menus)",
        "M: Toggle mute",
        "",
        "Shovel: click shovel then a planted tile to dig it up",
    ]
    def __init__(self, app, on_back=None):
        self.app=app
        self.on_back = on_back
    ...
    def draw(self,t):
        draw_day_sky(); self.app.draw_clouds()
        # Title
        text_out("Controls", FONT_XL, WHITE, center=(W//2, 60))
        # Card with help text
        card=pygame.Rect(40, 100, W-80, 200)
        pygame.draw.rect(screen, WOOD_LIGHT, card, border_radius=12)
        pygame.draw.rect(screen, WOOD_DARK, card, 2, border_radius=12)
        y=card.y+16
        for i,line in enumerate(self.LINES):
            font = FONT_L if i==0 else FONT_S
            text_out(line, font, BLACK, topleft=(card.x+14, y))
            y += 26 if i==0 else 20
        self.back.draw(); draw_grass_band(t)
    def handle(self,events):
        mouse=pygame.mouse.get_pos(); self.back.update(mouse)
        for e in events:
            ...
            if e.type==pygame.KEYDOWN and e.key==pygame.K_ESCAPE:
                self.back_action()
            if e.type==pygame.MOUSEBUTTONDOWN and e.button==1:
                if self.back.hover: self.back_action()
...
def make_beep(freq=660, ms=60, vol=0.2):
    ...
    try:
        ...
        # Numpy missing or buffer init failed.
        return None
...
class GameScene:
    ...
    def __init__(self, app):
        ...
        for idx,(name,cls,cost,cd,key) in enumerate(PLANT_TYPES):
            ...
            if x + 64 > W-80: # Avoid shovel
                # Non-fatal: seed might overlap shovel at small widths
                pass
    ...
    def _build_pause_ui(self):
        ...
        w, h = 220, 38
        self.pause_buttons = [
            Button((cx-w//2, y0 + 0*46, w, h), "Resume", self._pause_toggle),
            Button((cx-w//2, y0 + 1*46, w, h), "Controls", self._pause_controls),
            Button((cx-w//2, y0 + 2*46, w, h), "Options", self._pause_options),
            Button((cx-w//2, y0 + 3*46, w, h), "Restart Level", self._pause_restart),
            Button((cx-w//2, y0 + 4*46, w, h), "Quit to Menu", self._pause_quit),
        ]
        self.pause_panel = pygame.Rect(cx-140, cy-120, 280, 260)
    ...
    def _pause_quit(self):
        self.app.go(AdventureMap(self.app))
    def _pause_controls(self):
        cur = self
        def return_here():
            self.app.go(cur)
            self.paused = True
        self.app.go(Controls(self.app, on_back=return_here))
    def _pause_options(self):
        cur = self
        def return_here():
            self.app.go(cur)
            self.paused = True
        self.app.go(Options(self.app, on_back=return_here))
    # --- helpers ---
    ...
    def explode(self, r, c, radius):
        ...
        for z in self.zombies:
            if z.dead: continue
            rc = pos_to_cell(z.rect().centerx, z.rect().centery)
            if not rc: continue # Off-grid
            zr, zc = rc
    ...
    def update(self, dt):
        if self.paused:
            # while paused, allow mowers to idle (no motion) and freeze game entities
            return
        if self.state in ("win","lose"): 
            # allow animations to continue (mowers etc.); but stop gameplay
            for m in self.mowers: m.update(dt)
            # continue sky/clouds via app
            return
        self.cloud_phase += dt
        # cooldowns
    ...
    def draw(self, t, draw_ui=True):
        ...
        if draw_ui:
            ...
            if self.paused:
                ...
                screen.blit(ov,(0,0))
                # panel
                pygame.draw.rect(screen, WOOD_LIGHT, self.pause_panel, border_radius=12)
                pygame.draw.rect(screen, WOOD_DARK, self.pause_panel, 2, border_radius=12)
                text_out("Paused", FONT_L, BLACK, center=(self.pause_panel.centerx, self.pause_panel.y+24))
                # buttons
                for b in self.pause_buttons: b.draw()
                text_out("ESC to Resume", FONT_S, WHITE, center=(W//2, self.pause_panel.bottom+18))
    ...
    def handle(self, events):
        # update pause button hovers every frame
        mouse=pygame.mouse.get_pos()
        ...
        for e in events:
            ...
            if e.type==pygame.KEYDOWN:
                if e.key==pygame.K_ESCAPE:
                    # In win/lose, ESC behaves like clicking to continue (quit to adventure)
                    if self.state in ("win","lose"):
                        self.app.go(AdventureMap(self.app))
                    else:
                        self._pause_toggle()
                ...
                if e.key==pygame.K_m:
                    self.app.muted = not self.app.muted
                    continue
                # hotkeys 1-5 (disabled when paused)
                    ...
                    for i,s in enumerate(self.seed_bank):
                        if s["rect"].collidepoint(mx,my):
                            if self.seed_ready(i) and self.can_afford(i):
                                ...
                                self.shovel_selected=False
                                play(SND_CLICK)
                            # else: cooldown/insufficient sun -> ignored
                            return
                    # suns
                    for s in self.suns:
                        if s.rect().collidepoint(mx,my):
...
def main():
    ...
    while True:
        ...
        # Calculate delta time (clamped to avoid big spikes on focus changes)
        dt = clock.tick(FPS)/1000.0
        if dt > 0.05:  # cap at 50ms
            dt = 0.05
